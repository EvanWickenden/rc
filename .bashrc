...
alias clac='ssh eww2119@clac.cs.columbia.edu'


#compile and open as pdf .tex file
function pset (){ 
  if [ $# -eq 0 ]; then
    echo attach filename, pussy
  else
    pdflatex "pset$1.tex"
		rm -f "pset$1."{dvi,log,aux}
    open "pset$1.pdf"
  fi  
}
...
function runvim (){
	if [ $# -eq 0 ]; then
		echo attach filename, bitch
	else
		vim -c "source $1.vim"' | q!'
	fi
}

# END Columbia CS stuent settings
